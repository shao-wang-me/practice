...
def load_data() -> tuple[set[Tag], set[Challenge], set[Attempt]]:
  with open('data.json') as f:
    ...
    attemps = {Attempt(challenge=[c for c in challenges if c.name in x['challenge']][0], time=datetime.fromisoformat(x['time']), rating=x['rating']) for x in data['attempts']}
    return tags, challenges, attemps

def report(tags, challenges, attempts):
  ...
  for a in attempts:
    ...
    challenge_scores[a.challenge] += max(0, a.rating - (datetime.now() - a.time).days // 7)
